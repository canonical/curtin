...
def get_parted_info(devices=None):
    if devices is None:
        devices = []

    def dictfor(fields, data):
        ret = {}
    ...
    while i < len(lines):
        if lines[i] == "BYT;":
            cur = {'mode': "bytes", 'parts': []}
            cur.update(diskdata(lines[i+1]))
            i = i + 2
            while lines[i] != "":
                cur['parts'].append(partdata(lines[i]))
                i = i + 1
            ret.append(cur)
            i = i + 1
        else:
            errpos = lines[i].find("Error:")
            if errpos >= 0 and "unrecognised disk label" in lines[i]:
                disk = lines[i][errpos:].split()[1]
                cur = {'mode': 'error', 'path': disk}
                ret.append(cur)
                i += 1
            i += 1
    return ret
