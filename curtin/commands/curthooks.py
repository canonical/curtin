...
def detect_and_handle_multipath(cfg, target):
    ...
    # We usually want to overwrite bindings file provided by the image because
    # it was probably generated on another machine with different multipath
    # configuration. User can alter this behavior by setting overwrite_bindings
    # option to False. This may be useful if the image has been customized for
    # a specific machine and contains correct bindings.
    if mpbindings or not os.path.isfile(multipath_bind_path):
        # File /etc/multipath/bindings assigns permanent names to multipath
        # devices (in a form of '<name> <wwid>', where name usually looks like
        # /dev/mpathN). This file needs to be included into initramfs to avoid
        # spontaneous device name shuffling during boot. This file is usually
        # generated by multipathd daemon during installation of multipath-tools
        # package and gets included into initramfs automatically. In case of
        # curtin situation is different because curtin prevents any daemons
        ...
        # available in the install environment (one which is currently running)
        # due to a version mismatch.
        num_wwid = enumerate(block.get_multipath_wwids())
        multipath_bind_content = '\n'.join(
            ['# This file was created by curtin while installing the system.',
             '\n'.join(['mpath%s %s' % (num, wwid) for num, wwid in num_wwid]),
