...
def meta_simple(args):
    ...
    if len(devices) == 0:
        ...
        # checking for block devices and filtering out the removable ones.
        # In this case we may have a system which has its harddrives reported
        # by lsblk incorrectly. In this case we search for installable blockdevices
        # that are removable as a last resort before raising an exception.
        if len(devices) == 0:
            devices = block.get_installable_blockdevs(True)
            LOG.warn("No non-removable, installable devices found. "
                     "List populated with removable devices allowed: %s", devices)
            # Fail gracefully if no devices are found, still.
            if len(devices) == 0:
                raise Exception("No valid target devices found that curtin can install on.")
